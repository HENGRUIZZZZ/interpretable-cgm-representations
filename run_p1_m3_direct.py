"""
P1 M3：简单 NN 消融。仅 Encoder (LSTM) + 预测头，无 VAE、无 Decoder。

与 run_p1_full_pipeline 共用数据加载与划分；训练时只优化 IR/SSPG/DI 预测 loss，无重建与 KL。
主结果来自测试集上的端到端 head 评估，保存至 e2e_head_metrics.json。

用法（项目根目录）：
  export CGM_PROJECT_OUTPUT=/path/to/cgm_project/output
  export LAMBDA_SSPG=1.0 LAMBDA_DI=1.0
  python run_p1_m3_direct.py
"""
import os
import sys
import json
...
RESULTS_DIR = os.environ.get("P1_RESULTS_DIR", "paper1_results_m3")
...
def main():
    ...
    from models import DirectNN, count_params
    ...
    print("M3 DirectNN (Encoder + heads only). Device:", device)
    ...
    sspg_head = torch.nn.Linear(len(PARAM_NAMES), 1).to(device)
    di_head = torch.nn.Linear(len(PARAM_NAMES), 1).to(device)
    optimizer = torch.optim.AdamW(
        list(model.parameters()) + list(ir_head.parameters()) + list(sspg_head.parameters()) + list(di_head.parameters()),
        lr=LR,
    )
    print("Params:", count_params(model), "(+ heads)")
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        for tup in train_loader:
            ...
            optimizer.zero_grad()
            out = model(cgm, timestamps, meals, demographics)
            param = out.param
            loss = torch.tensor(0.0, device=device)
            valid = torch.isfinite(homa_ir_b) & (homa_ir_b >= 0)
            if valid.any():
                ir_in = param[:, IR_LATENT_IX]
            ...
            if LAMBDA_SSPG > 0:
                ...
                if valid_sspg.any():
                    loss = loss + LAMBDA_SSPG * (sspg_head(param).squeeze(-1)[valid_sspg] - sspg_b[valid_sspg]).pow(2).mean()
            if LAMBDA_DI > 0:
                valid_di = torch.isfinite(di_b)
                if valid_di.any():
                    loss = loss + LAMBDA_DI * (di_head(param).squeeze(-1)[valid_di] - di_b[valid_di]).pow(2).mean()
            loss.backward()
            optimizer.step()
    ...
    with torch.no_grad():
        ...
        sspg_head.eval()
        di_head.eval()
        out_test = model(*test_tensors[:4])
        param_test = out_test.param.cpu().numpy()
        lat = torch.as_tensor(param_test, dtype=torch.float, device=device)
        sspg_pred_e2e = sspg_head(lat).squeeze(-1).cpu().numpy()
        di_pred_e2e = di_head(lat).squeeze(-1).cpu().numpy()
    ...
    print("\n--- End-to-End Head Evaluation on Test Set (M3) ---")
    ...
    print("Saved", RESULTS_DIR, "e2e_head_metrics.json")
