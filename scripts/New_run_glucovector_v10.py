...
OUT_ROOT = os.path.join(REPO_ROOT, "New_paper1_results_glucovector_v10")
...
def _predict_per_meal(ckpt_path: str, cal_sspg: Tuple[float, float], cal_di: Tuple[float, float], out_csv: str) -> None:
...
def _run_one_experiment(name: str, env_cfg: Dict[str, str]) -> Dict[str, object]:
    ...
    m_sspg = _apply_calibration(
        os.path.join(exp_dir, "D4_sspg_true_vs_pred_raw.csv"), a_s, b_s,
        os.path.join(exp_dir, "D4_sspg_true_vs_pred.csv"), os.path.join(exp_dir, "D4_sspg_metrics.json")
    )
    m_di = _apply_calibration(
        os.path.join(exp_dir, "D4_di_true_vs_pred_raw.csv"), a_d, b_d,
        os.path.join(exp_dir, "D4_di_true_vs_pred.csv"), os.path.join(exp_dir, "D4_di_metrics.json")
    )

    # Required delivery: per-subject per-meal predictions
    _predict_per_meal(ckpt, (a_s, b_s), (a_d, b_d), os.path.join(exp_dir, "per_subject_per_meal_preds.csv"))

    # Required delivery: SHAP summary for SSPG
    subprocess.run(
        [
            "python",
    ...
    return {"name": name, "sspg": m_sspg, "di": m_di}
...
def run_v10() -> None:
    ...
    for name, cfg in experiments:
        res = _run_one_experiment(name, cfg)
        summary_rows.append(
            {
                "experiment": name,
                "sspg_n": res["sspg"]["n"],
                "sspg_pearson_r": res["sspg"]["pearson_r"],
                "sspg_r2": res["sspg"]["r2"],
                "sspg_rmse": res["sspg"]["rmse"],
                "di_n": res["di"]["n"],
                "di_pearson_r": res["di"]["pearson_r"],
                "di_r2": res["di"]["r2"],
                "di_rmse": res["di"]["rmse"],
            }
        )
    ...
    summary_csv = os.path.join(OUT_ROOT, "v10_experiment_summary.csv")
    summary_df.to_csv(summary_csv, index=False)

    rep = ["# v10_comprehensive_report", "", f"Run stamp: {stamp}", "", "## D4 calibrated summary", summary_df.to_markdown(index=False), ""]
    rep += ["## Output structure", "- One folder per experiment under `New_paper1_results_glucovector_v10/`", "- Each folder includes metrics, scatter CSVs, latent 26D CSV, SHAP outputs, and per-subject per-meal predictions."]
    with open(os.path.join(OUT_ROOT, "v10_comprehensive_report.md"), "w", encoding="utf-8") as f:
        f.write("\n".join(rep))

    zip_path = os.path.join(REPO_ROOT, "New_paper1_results_glucovector_v10.zip")
    subprocess.run(["/bin/zsh", "-lc", f'cd "{REPO_ROOT}" && zip -r "{zip_path}" "New_paper1_results_glucovector_v10" >/dev/null'], check=False)
